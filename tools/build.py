...
# Caps data/logs/{tag}.log from growing unboundedly on a long-running
# self-hosted install (confirmed live: no rotation existed at all, so a
# multi-day session could grow these to unbounded size, both wasting disk
# and making apps/web's GET /api/health/logs tail slower/heavier over
# time). Coarse "shrink when too big" truncation, not a proper
