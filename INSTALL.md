...
| Git | `git --version` | |
| Docker Desktop (Windows) / Docker Engine + Compose (Linux) | `docker --version` | Solo si vas a usar Postgres local (Paso 6, opción A). Si vas a usar una DB remota (opción B), sáltalo. |
...
- (Opcional, recomendado) **WSL2**, si quieres que la verificación
...
datos remota, sigue el Paso 6, opción B, que te dice exactamente qué
líneas cambiar.
...
No hace falta correr las migraciones a mano aquí — el wizard de `/setup`
(Paso 8) las corre por ti. Sigue con el Paso 7.
...
Necesitas Postgres 14+ con `postgis` y `vector` (pgvector) disponibles, y
...
Neon, activa ambas extensiones desde su panel antes de seguir. En RDS,
necesitas Postgres 16+ para pgvector.
...
**6B.4 — (Opcional) Si vas a correr los tests del monorepo**, crea también
una base `netryx_test` en el servidor remoto (a mano, con tu cliente de
Postgres de preferencia) y repite el comando del paso anterior apuntando
...
remoto. Como ya migraste a mano aquí, ese paso del wizard no hace nada útil
para ti (no rompe nada, simplemente no aplica).
...
No hace falta instalar nada de Python a mano — simplemente sigue el
...
3. Crea el entorno virtual de `services/inference` e instala sus
   ...
   dentro de WSL2 en vez de nativo — úsalo solo si ya instalaste WSL2 tú
   mismo, Paso 1).
...
resultado) cuando le compartes una imagen y le preguntas dónde fue tomada.

**Si corres Claude Code con este repo como directorio de trabajo**, no
hace falta instalar nada — Claude Code detecta automáticamente cualquier
skill dentro de `.claude/skills/` del proyecto abierto.

**Si quieres que esté disponible en cualquier sesión de Claude Code, sin
importar el directorio**, cópiala a tu carpeta global de skills:
...
cuando le compartes una foto y le preguntas dónde fue tomada, siempre que
...
| La app conecta bien pero las tablas nunca existen | Las migraciones corrieron contra `localhost` en vez del servidor remoto | Repite el Paso 6B.3 con los datos correctos |
