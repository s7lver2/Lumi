...
/** Vive en `ui/`, no en `admin/`: `App.tsx` la monta para toda la app (una
 *  sola vez, junto a la barra de título), no solo dentro del panel de
 *  administración — un usuario normal bloqueado por el modo también tiene
 *  que enterarse de por qué. Rayas diagonales deslizándose despacio — la
 *  lectura visual es "cinta de obra en movimiento", no decoración: mientras
 *  se mueve, el modo sigue activo. Reutiliza `jg-alert-pulse` (ya existe en
 *  index.css) para el pulso del icono en vez de definir una animación nueva
 *  para eso. */
