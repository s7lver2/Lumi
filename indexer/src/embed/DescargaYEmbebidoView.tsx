 ...
 *  co-renderizadas (#63): mientras `descargando` es verdad se ve solo
 *  `DownloadView` a pantalla completa, aunque ya haya trabajo de embebido
 *  encolado para fotos de un origen que terminó antes que otro — enseñarlo a
 *  medias se leía como "solo trajo imágenes de un proveedor" (#62). El
 *  `trabajando` del backend ya solo se apaga cuando TODOS los orígenes
 *  activos terminaron (`download.rs::correr`), así que basta con esperar a
 *  que `descargando` pase a falso — no hace falta mirar `por_origen` aquí.
 *  En cuanto pasa a falso (`onTerminadoDescarga`, disparado desde
 *  `DownloadView`), el padre dejar de pasar `descargando` y aquí se pinta el
 *  embebido a pantalla completa.
 *
