...
- [x] `cargo test -p indexer-app` limpio: 101 tests. `cargo test` desde la raíz: todo verde salvo `lumi-installer::marca::tests::cliente_e_indexer_son_entradas_independientes`, que es un **fallo previo y ajeno a este plan** — sus tres tests comparten la misma clave de registro (`RAIZ_PRUEBA`) y se pisan al correr en paralelo; con `--test-threads=1` pasan los 11. Ningún commit de este plan toca `crates/`.
- [x] `cd indexer && npm run build && npm run lint` limpio. Trozo principal: 2.320,27 kB → 473,00 kB.
- [ ] **PENDIENTE, requiere ojos del operador.** Prueba manual de extremo a extremo: abrir la app con Qdrant en frío (o simulado), confirmar entrada inmediata a Proyectos; activar el interruptor de descarga paralela y lanzar una descarga de prueba con al menos dos orígenes activos, confirmar que corren a la vez (verlo en el registro de la pantalla de Descarga) y que el resultado final (imágenes, gasto, teselas) es correcto; abrir Revisión y confirmar miniaturas; abrir Ajustes → Rendimiento y confirmar el botón de migración de Qdrant y el aviso de disco mecánico si aplica.
- [x] Repasado: ningún `TODO`/`FIXME` nuevo en el diff, y cada commit cierra su tarea. Los que comparten fichero son Task 4/5 (`download.rs`), 5/6/10 (`origins/mod.rs`) y 2/3/5 (`store.rs`), en ese orden; el historial se lee en orden aunque revertir uno suelto de esos pares exija mirar al vecino. Repasar que ningún commit dejó `TODO`/comentario a medias, y que cada uno cierra su propia tarea de forma independiente (se puede revertir uno sin romper los demás, salvo Task 4/5/6 que comparten `download.rs`/`origins/mod.rs` — revisar que el historial quede legible igualmente).
