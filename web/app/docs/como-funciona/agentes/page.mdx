...
<Detalle titulo="Los números exactos">
  ...
  completo declarado en <Simbolo ruta="registros/niveles/pro.json">registros/niveles/pro.json</Simbolo> —
  idioma y señalética, condiciones ambientales, hora y sombras, tipo de escena, dimensiones de la vía,
  entre otros. Cada uno es su propio registro bajo `registros/agentes/`.
</Detalle>

<Detalle titulo="Qué hace esto en el código">
  <Simbolo ruta="crates/lumid/src/agentar.rs">crates/lumid/src/agentar.rs</Simbolo> ejecuta cada
  agente activo sobre la imagen de entrada y devuelve su distribución de verbalizadores; esa
  distribución no descarta ninguna hipótesis por sí sola, solo aporta un factor que la siguiente
  etapa combina con las demás — ver «El veredicto y su confianza».
</Detalle>
