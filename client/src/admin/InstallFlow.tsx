...
/** La tarjeta de progreso, aparte de `InstallFlow`: `IndicesPanel` la
 *  reutiliza tal cual para una instalación que ya estaba en marcha antes de
 *  entrar a esta pantalla (el toast "ver progreso →" solo navegaba aquí sin
 *  enseñar nada, porque el `progreso` de `InstallFlow` nacía siempre vacío y
 *  únicamente se rellenaba al confirmar una URL nueva). */
export function ProgresoInstalacionCard({ progreso, onCerrar }: {
  progreso: ProgresoInstalacion;
  onCerrar: () => void;
}) {
...
export function InstallFlow({ token, onCerrar, onInstalado }: {
  ...
  if (progreso) {
    return <ProgresoInstalacionCard progreso={progreso} onCerrar={onCerrar} />;
  }
