...
**New `apps/web/app/setup/steps/ModelsStep.tsx`**: takes `useCases: UseCaseId[]` and `runtime: "windows" | "linux" | "wsl"` (the same runtime value `SetupWizard.tsx` already computes for `InstallStep` — threading it here too, since which weight-download step ids are valid depends on it) collected from earlier steps, computes `const bundles = recommendedBundles(useCases); const toInstall = bundles.length > 0 ? bundles : MODEL_BUNDLES;`. Renders one recommendation blurb per bundle ("Recomendado para: reconocimiento de imágenes, geolocalización" — built by reverse-mapping `useCases` against `RECOMMENDATIONS_BY_USE_CASE`, or "Recomendado" alone if using the empty-selection fallback), then a checklist reusing the existing `InstallItem` component: for `lumi-preview` specifically, the checklist items are `weights-retrieval`/`weights-verification` (or their `-wsl` siblings when `runtime === "wsl"`, matching what `InstallStep.tsx`'s `ITEMS_BY_RUNTIME.wsl` used before this plan) followed by `verify-services` (a single fixed id regardless of runtime — same `stepId`s as before, run via the same `useCommandRun`/`run/[step]` SSE mechanism — `InstallItem` is reused unmodified). `onComplete()` fires once all items are done, same pattern as `InstallStep`.

**`SetupWizard.tsx`**: add `usage`/`models` to the `panel` map and `SUBTITLE` record (`usage: "para qué vas a usar Lumi."`, `models: "instalando lo recomendado."`); the existing `collected` state is a `Record<string, string>` (string values only), so the Uso step's array selection needs its own state — add `const [useCases, setUseCases] = useState<UseCaseId[]>([])` alongside the existing `collected`/`done`/`dirty`-equivalent state, wire `UsageStep`'s `selected`/`onSelectedChange` to it, and pass `useCases` into `ModelsStep`. Also pass `ModelsStep` the same `runtime` value already computed for `InstallStep` — extract that computation into a single local variable used by both, rather than duplicating the ternary.
