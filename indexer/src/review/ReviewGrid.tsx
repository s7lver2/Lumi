  ...
  return (
    <div className="flex h-full">
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center gap-1.5 p-[16px_18px_12px]">
        ...
        <Rejilla visibles={visibles} fuera={fuera} onClic={clic} />
      </div>

      <aside className="w-[300px] shrink-0 border-l border-border bg-[rgba(16,18,21,.5)] p-[20px_18px]">
...
/** La rejilla, VIRTUALIZADA. Sin esto, `visibles.map()` montaba un `<button>`
 *  y un `<img>` por cada foto pendiente: una descarga normal deja decenas de
 *  miles (26.325 en el índice que destapó esto), y el navegador se cae mucho
 *  antes de llegar al final. `loading="lazy"` no salva de esto — retrasa la
 *  carga del `src`, pero el nodo se monta igual, y el coste está en el DOM,
 *  no en la red.
 *
 *  Es el mismo arreglo, con el mismo `@tanstack/react-virtual`, que
 *  `catalog/IndexMapDialog.tsx` ya aplicó a su galería por la misma razón —
 *  incluido el `measure()` del efecto de abajo, que allí está explicado a
 *  fondo: el virtualizador cachea la altura de la primera fila que toca, y en
 *  el primer render (antes de que el `ResizeObserver` mida) esa altura es
 *  ~10px, así que sin recalcular las filas se dibujan unas sobre otras. */
function Rejilla({ visibles, fuera, onClic }: {
  visibles: FichaRevision[];
  fuera: Set<number>;
  onClic: (lista: FichaRevision[], i: number, conMayus: boolean) => void;
}) {
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const medir = () => {
      const ancho = el.clientWidth - 36; // p-[0_18px] a cada lado = 36px
      setAnchoColumna(Math.max(0, (ancho - HUECO * (COLUMNAS - 1)) / COLUMNAS));
    };
    medir();
  ...
  const filas = Math.ceil(visibles.length / COLUMNAS);
  ...
  }, [altoFila, visibles.length]);

  return (
    <div ref={contenedorRef} className="flex-1 overflow-y-auto p-[0_18px_16px]">
      {anchoColumna > 0 && (
        <div style={{ position: "relative", height: virtualizador.getTotalSize(), width: "100%" }}>
          {virtualizador.getVirtualItems().map((fila) => {
            ...
            return (
              ...
              >
                {visibles.slice(inicio, inicio + COLUMNAS).map((f, k) => {
                  const i = inicio + k;
                  const no = fuera.has(f.id);
                  return (
                    <button
                      key={f.id}
                      onClick={(ev) => onClic(visibles, i, ev.shiftKey)}
                      aria-pressed={no}
